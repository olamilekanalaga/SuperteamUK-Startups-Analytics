...
export function reviewedNarrativeQueries({ id, kind, chart, queryId, queryIds, sourceRowsByQuery }, queries) {
  ...
  if (ids.length > 1 && (chart || !["narrative", "custom"].includes(kind))) {
    throw new Error(`DataComponent "${id}" supports multiple reviewed queries only for narrative components.`);
  }
