...
import { copyChartImage, downloadChartImage } from "./chart-image.js";
import {
...
import { Dialog, Tooltip } from "./components/ui.jsx";
...
export function DataAppShell({
  ...
  const registerDashboardTabs = useCallback(
    (definitions) => {
      ...
      if (!initialAuthoredTabsRegistered.current) {
        ...
        const state = readDashboardUrlState(snapshot, authored, initialDashboardHref);
        setActiveTabId(state.tab ?? (uniqueIds.has(activeTabRef.current) ? activeTabRef.current : nextTabs[0].id));
  ...
  const editHistory = usePresentationHistory(presentation, canEdit && mode === "edit", (value) => {
  ...
  const saveStatus = usePresentationPersistence({
    ...
    canEdit,
    presentation,
    personalPresentation: hosted ? { filters, assumptions } : {},
  ...
  useEffect(() => {
    ...
    const next = dashboardUrlWithState(window.location, snapshot, tabs, {
      tab: activeTabId,
      filters,
    });
  ...
  }, [activeTabId, filters, reportSurface, snapshot, tabs, tabRegistrationReady]);

  useEffect(() => {
    if (reportSurface || typeof window === "undefined") return undefined;

    function restoreDashboardUrlState() {
      const state = readDashboardUrlState(snapshot, tabs, window.location);
      setActiveTabId(state.tab ?? tabs[0].id);
      replaceFilters(resolveDashboardUrlFilters(snapshot, filtersRef.current, state, { reset: true }));
    }
  ...
  const navigateDashboardTab = useCallback(
    (tabId, nextTabs = tabs) => {
      if (tabId === activeTabRef.current) return;
      if (!reportSurface && typeof window !== "undefined") {
        const next = dashboardUrlWithState(window.location, snapshot, nextTabs, {
          tab: tabId,
          filters: filtersRef.current,
        });
        if (next) next.pathname = canonicalDashboardPath(next.pathname);
        if (next && next.href !== window.location.href) {
          window.history.pushState(window.history.state, "", next);
          // A deliberate tab change leaves the component route. pushState does
          // not emit popstate, so clear its pending navigation and highlight here.
          if (hosted) followComponentPermalink();
        }
      }
      setActiveTabId(tabId);
    },
    [followComponentPermalink, hosted, reportSurface, snapshot, tabs],
  );
  ...
  useEffect(() => {
    if (!tabRegistrationReady || !hosted || !linkedComponent) return;
  ...
  async function copyComponent(format, component) {
    ...
    if (format === "download-image") {
      try {
        await downloadChartImage(component.id, {
          description: component.description,
          title: component.title,
        });
        setActionStatus(`Chart image for “${component.title}” downloaded as PNG.`);
      } catch (error) {
        setActionStatus(error instanceof Error ? error.message : "Unable to download the chart image.");
      }
      return;
    }
