...
const objectKeys = ["componentTitles", "textEdits", "chartOverrides", "filters", "assumptions", "blockLayouts"];
...
export function validatePresentation(value) {
  ...
  if (value.refreshSchedule !== undefined && !normalizeDataAppRefreshSchedule(value.refreshSchedule)) {
    throw new Error("Refresh schedule must include a valid repeat, time, and selected days.");
  }
...
export function mergePresentationChanges(previous, next, latest) {
  const result = { ...normalizePresentation(latest) };
  for (const key of allowedKeys) {
  ...
  return normalizePresentation(result);
}
