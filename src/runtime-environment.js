...
export function chatGPTPromptUrl(
  ...
  const desktop = local || isCodexBrowser(userAgent);
...
// Published Ask ChatGPT controls hand off through ChatGPT Web so that one
// public URL can launch Codex Desktop or retain the Work-mode web fallback.
export function chatGPTWorkTaskUrl(prompt, browserUrl) {
  if (typeof prompt !== "string" || !prompt.trim()) {
    throw new Error("A new Data app task requires a prompt.");
  }
  const url = new URL("/codex/open-app", "https://chatgpt.com/");
  url.searchParams.set("app_brand", "chatgpt");
  url.searchParams.set("fallback", "work");
  url.searchParams.set("q", prompt);
  const sourceUrl = safeSourceHref(browserUrl);
  if (sourceUrl) url.searchParams.set("browserUrl", sourceUrl);
  return url;
}
