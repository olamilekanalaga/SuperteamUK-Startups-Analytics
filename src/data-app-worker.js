...
export function createDataAppWorker({
  ...
  async function viewerCanEdit(request) {
    // A configured stable-ID deployment never downgrades to email when a
    // request lacks its ID. Simultaneous or malformed seeds fail closed.
    if (dataAppOwnerUserIdSha256 !== "") {
      if (dataAppOwnerEmailSha256 !== "" || !isOwnerHash(dataAppOwnerUserIdSha256)) return false;
      const userId = request.headers.get("oai-authenticated-user-id")?.trim();
      if (!userId || userId.length > 512 || /[\s\0-\x1f\x7f]/u.test(userId)) return false;
      return (await sha256(userId)) === dataAppOwnerUserIdSha256;
    }

    if (!isOwnerHash(dataAppOwnerEmailSha256)) return false;
    // Only the Sites dispatcher may supply this header. Packaging enables this
    // temporary mode from fresh, canonical, private owner metadata, never from
    // an arbitrary browser visitor or a client-provided email.
    const email = authenticatedViewerEmail(request);
    if (!email || email.length > 254 || /[\0-\x1f\x7f]/u.test(email) || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(email))
      return false;
    return (await sha256(email)) === dataAppOwnerEmailSha256;
  }
    ...
    async fetch(request, environment) {
      ...
      if (pathname === "/api/presentation" && request.method === "PUT") {
        ...
        if (body.verificationAction === "remove") {
          ...
          if (current.presentation.verification) presentation.verification = current.presentation.verification;
          else {
            try {
              presentation.verification = validatePresentation({
                verification: { verifiedBy: authenticatedViewerEmail(request), verifiedAt: updatedAt },
              }).verification;
            } catch {
              return json({ error: "A valid authenticated owner email is required to verify this Data app." }, 400);
            }
          }
