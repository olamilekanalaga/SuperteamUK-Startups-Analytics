...
export function histogram(rows, field) {
  ...
  return [...counts]
    .map(([start, count]) => ({ start, count, end: start + width }))
...
export function heatmap(rows, x, y, value, { domain, startAtZero } = {}) {
  ...
  return {
    rows: xValues.flatMap((column, xIndex) =>
      yValues.map((group, yIndex) => {
        ...
        const row = source ?? { [x]: column, [y]: group, [value]: 0, __missing: true };
        return {
          ...row,
          xIndex,
          yIndex,
...
export function sankeyGraph(rows, stages, value) {
  ...
  for (const row of rows) {
    ...
    for (let index = 0; index < fields.length - 1; index += 1) {
      const sourceValue = row[fields[index]];
      const targetValue = row[fields[index + 1]];
      ...
      const source = node(index, sourceValue);
      const target = node(index + 1, targetValue);
