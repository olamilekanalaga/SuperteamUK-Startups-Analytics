...
const chartsWithoutAxes = new Set(["pie", "funnel", "sparkline", "histogram", "boxPlot", "rankedList"]);
...
function ChartSwitch({ label, checked, onChange }) {
  return <button type="button" className="explorer-toggle" role="switch" aria-label={label}
    aria-checked={checked} onClick={() => onChange(!checked)}><span>{label}</span>
    <span className="explorer-switch" aria-hidden="true" />
  </button>;
}
...
export function ChartExplorer({
  ...
  const availableTypes = chartTypes.filter((type) => type !== "leaderboard"
    && (component.chart.stackable !== false || !type.toLowerCase().includes("stacked")));
  ...
  const update = (field) => (value) => {
    ...
    const next = {
      ...
      ...(field === "type" ? { preserveBarChart: value === "horizontalBar" } : {}),
  ...
  return <div className="chart-explorer">
    ...
    {canEdit && <div className="explorer-controls" aria-label="Chart controls">
      ...
      {!chartsWithoutAxes.has(spec.type) && <section className="explorer-section" aria-label="Chart axes settings">
        ...
        {spec.type !== "heatmap" && <ChartSwitch label="Start axis at zero" checked={spec.startAtZero !== false}
      ...
      {(!chartsWithoutOptionalLabels.has(spec.type) || hasLegend || hasAnnotations) &&
        <section className="explorer-section" aria-label="Chart appearance settings">
          <h3>Appearance</h3>
          {!chartsWithoutOptionalLabels.has(spec.type) && <ChartSwitch label="Show values"
            checked={spec.showValues === true} onChange={update("showValues")} />}
          {hasLegend && <ChartSwitch label="Show legend"
