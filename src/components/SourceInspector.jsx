...
function recordedEvidence({ component, source, definitions, rows, activeFilters, dateRange }) {
  const freshness = source.executedAt;
  const evidence = [
    {
      title: "Recorded data source",
      detail: [source.label, ...source.tables, ...source.files.map(({ label }) => label)].filter(Boolean).join(" · "),
    },
    {
      title: "Captured snapshot",
      detail: [
        `${rows.length} ${rows.length === 1 ? "row" : "rows"} in this component's recorded scope`,
        freshness ? `query executed ${formatSnapshot(freshness)}` : "",
  ...
  const filters = [...source.filters, ...activeFilters];
  if (filters.length) evidence.push({ title: "Recorded filters", detail: filters.join(" · ") });
  if (definitions.length)
    evidence.push({
      title: "Provided metric definitions",
      detail: definitions.map(({ label, definition }) => `${label}: ${definition}`).join(" · "),
    });
  if (source.sql)
    evidence.push({
      title: "Recorded SQL query",
      detail: "The exact query saved with this snapshot is available in the SQL query tab.",
    });
  evidence.push({
    title: "Displayed component",
    detail: `${component.title} · ${component.kind} · ${component.queryId}`,
  });
  return evidence;
}
...
function SourceDetails({ component, query, rows, filters, allowCopy, receipt, section }) {
  ...
  const chart = chartOverride ?? component.chart;
  ...
  const { sql, tables, tableLinks, tableTrust, files, definitions, links, caveats, evidenceFlow } = source;
  const visibleDefinitions = scopedMetricDefinitions(definitions, component.id, {
    displayedFields,
    chartEdited: Boolean(chartOverride),
  });
  const dateRange = receipt ? receipt.reportingPeriod : reviewedDateRange(rows, { component, query, filters });
  const freshness = source.executedAt;
  ...
  const explicitEvidence = evidenceFlow.length > 0;
  const evidence = receipt ? evidenceFlow : explicitEvidence
    ? scopedProvidedEvidence(evidenceFlow, visibleDefinitions, activeFilters)
    : recordedEvidence({
        component,
        source,
  ...
  return (
    <>
      ...
      {section === "overview" && <>
        {!receipt && <dl className="source-metadata">
          {!!dateRange && (
            <div>
        ...
        {!!(receipt || tables.length || files.length || links.length) && (
          <div className="source-group">
            <p className="source-label">Sources</p>
            <div className="source-list">
              {receipt && !tables.length && !files.length && !links.length && <SourceRow receipt={Boolean(receipt)} label={source.label} />}
              {tables.map((table) => (
                <SourceRow
      ...
      {section === "evidence" && <>
        <ol className="source-trace" data-evidence-origin={explicitEvidence ? "provided" : "recorded-metadata"}>
...
export function SourceInspector({ component, query, rows, filters, allowCopy = true, receiptQueries, onReceiptCollapse, receiptContentId, externalReceiptToggle = false }) {
  ...
  const hasData = !receipt || queries.some((entry) => hasReceiptSection(entry, "data"));
  const hasSql = !receipt || queries.some((entry) => hasReceiptSection(entry, "sql"));
  const hasEvidence = !receipt || hasReceiptEvidence(consolidated);
  const tabs = [
  ...
  const panels = tabs.map(({ id }) => {
    ...
    return <TabPanel key={id} keepMounted={receipt} tabsId={tabsId} tabId={id}
      active={tab === id} className={`source-section${receipt && id === "overview" ? " receipt-overview" : ""}${receipt && id === "sql" && entries.length === 1 ? " receipt-sql-panel" : ""}`}>
      {receipt && id === "overview" ? <>
        {component.description && <p className="receipt-description">{component.description}</p>}
        <ReceiptOverview overview={consolidated} />
  ...
  const tabBar = <Tabs
    ...
    items={tabs} value={tab} onChange={setTab}
  ...
  return <div ref={inspectorRoot} className="source-inspector">
    ...
    {receipt ? <ReceiptPanels activeTab={tab}>{panels}</ReceiptPanels> : panels}
  </div>;
