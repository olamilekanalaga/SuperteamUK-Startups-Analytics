...
export function SortableRegion({
  ...
  const stableAuthoredIds = board
    ? [...(authoredOrder ?? localElements.map((item) => item.props.id))
      .filter((itemId) => authoredIds.includes(itemId)),
    ...incoming.map((item) => item.props.id)]
    : authoredOrder ?? authoredIds;
  if (authoredIds.some((itemId) => !validBlockLayoutId(itemId))
    || new Set(authoredIds).size !== authoredIds.length
    ...
    || authoredIds.some((itemId) => !stableAuthoredIds.includes(itemId))) {
