...
const ownerExportNames = ["dataAppOwnerUserIdSha256", "dataAppOwnerEmailSha256"];
...
function normalizeOwner(bytes, path) {
  ...
  const values = [];
  let offset = 0;
  for (const name of ownerExportNames) {
    const prefix = `export const ${name} = "`;
    const exportStart = skipJavascriptTrivia(source, offset);
    const start = exportStart + prefix.length;
    const end = source.indexOf('"', start);
    if (exportStart < 0 || !source.startsWith(prefix, exportStart) || end < 0 || !source.startsWith('";', end)) {
    ...
    values.push({ start, end, value });
    offset = end + 2;
  }
  if (skipJavascriptTrivia(source, offset) !== source.length) {
    throw new Error("The protected Data app owner module must contain exactly its expected owner exports.");
  }
  ...
  // Publication may seed, transfer, revoke, or migrate the verified owner. The
  // two exports' syntax, comments, and every other source byte remain protected.
  const normalized = [];
  offset = 0;
